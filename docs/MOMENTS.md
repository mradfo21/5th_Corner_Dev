...
| Library stinger | `static/audio/library/stinger/encounter_*.mp3`, served as `/static/audio/library/…` | The twelve encounter one-shots, shipped in the sound library (made on ElevenLabs before 2026-09-25 and re-levelled into it; nothing generates them at runtime — `docs/plans/ONE_KEY_AUDIO_PLAN.md`). Keyed by the ids `Sound.STOCK_CUE` maps; `scene_audio.encounter_stinger_urls()` / `/api/music/stock`. |
...
Both are library tracks (`sound_library.pick`, mode `encounter`): hostile /
desperate / opportunistic / creature each have their own bed, a creature
wins over the stance, and a critical run gets the fight-for-your-life one.
The ambience under it is the place the fight is in (the frame's vision read).
...
| Beat | Cue | Also |
|------|-----|------|
| Hitch (~520ms) | `encounterHitch` | heartbeat 76, the library's confrontation bed + the place's ambience, duck 0.22 |
