...
**Core Loop:** See image → Read dispatch → Choose action → Consequence → New image → Repeat
...
- **Reference Images:** 
  - **1 reference image** → More dramatic change (for action choices like "climb," "run," "attack")
  - **2 reference images** → More continuity (for stationary choices like "wait," "examine," "speak")
- **Prompt Sanitization:** Violent/graphic terms are replaced with clinical equivalents to bypass Gemini safety filters
  - Example: "blood splatter" → "tissue damage," "severed limb" → "traumatic amputation"
...
2. Calculate consequence (LLM analyzes risk → outcome)
...
- If no choice picked → **Timeout Penalty** injected as player action
...
- When Auto-Play is ON → Countdown is **DISABLED**
...
- On death or restart → Creates GIF from all images
- GIF settings: 500ms per frame (2x speed), loop forever
- Saved to `tapes/tape_{timestamp}.gif`
- Posted to Discord with "📼 VHS TAPE" embed

**Use Cases:**
- Player dies → Tape generated automatically
- Player presses Restart → Tape generated before reset
...
- **Preserve the core loop** (image → dispatch → choices → consequence)
